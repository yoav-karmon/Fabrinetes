...
def restore(args, container_info):
    ...
    print_aligned_comment("# docker load -i", "# Docker load command", comment_column)
    print_aligned_comment(f"#     {tar_path}", "# Tarball path (from config.image.tarball_path)", comment_column)
    ...
    if resolved_tar_path is None:
        ...
        tarball_name = container_info.image_tarball
