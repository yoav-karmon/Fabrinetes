...
USERNAME=${CONTAINER_USER:-yoav.karmon}
USER_UID=${CONTAINER_UID:-1898250003}
USER_GID=${CONTAINER_GID:-1898250003}
...
# If no arguments are passed (e.g., from postCreateCommand), just exit successfully
if [ $# -eq 0 ]; then
    echo "No command arguments provided, user setup complete."
    echo "User $USERNAME is ready for use."
    exit 0
fi

# If arguments are provided, switch to the user and execute the command
